...
pinMode(fan_1, OUTPUT); analogWrite(fan_1, 191); //PWM enabled pin
/*
if(ESP32){
...
*/
 ...
 do{ digitalWrite(sound, HIGH); digitalWrite(flashin_LED, LOW); delay(50); digitalWrite(sound, LOW); digitalWrite(flashin_LED, HIGH); delay(50); beepu++; } while(beepu < 2);
  ...
  //  ledcWrite(channel_1, Speed_0); ledcWrite(channel_2, Speed_0);
    ...
    analogWrite(fan_1, 0); analogWrite(fan_1, 0);
...
*/
  ...
  if(!Coil_1_ON) {if(batt_runs%100000 == 0) MonitorBattery();  // Query the battery once every second since it doesn't change that rapidly
      }
  else { //if either coil 1 is on or 2 is ON or BOTH
                                if(batt_runs%50000 == 0) MonitorBattery();
    }

  Monitor_cookingZones(heat_1_level);
  //listen_to_timer_set()
   batt_runs++;
...
unsigned long fan_1_sat   = 0;
unsigned long fan_1_stop  = 0;
...
void Monitor_cookingZones(uint8_t coil_1_index){ //COIL INDEX IS THE MASTER CONTROLLER 
...
//close if power is set to zero
    ...
    */
            if(!waiting){//turn it on instantly but very silently... for the first 30 seconds
                          if(!fan1_ON) { analogWrite(fan_1, 63); /*ledcWrite(channel_1, Speed_1);*/ fan1_ON = true; /* Serial.print("Fan 1 Speed: "); Serial.println(Speed_1); */ fan1_speed_2_engaged = false; fan1_speed_4_engaged = false; }
                          if(fan1_ON){
                            if(((millis() - stat) >= 20000) && (millis() - stat) < 40000){ // for ... 20 seconds
                                if(!fan1_speed_2_engaged) {analogWrite(fan_1, 191); /*ledcWrite(channel_1, Speed_2); /* Serial.print("Fan 1 Speed: "); Serial.println(Speed_2);  */ fan1_speed_2_engaged = true;  fan1_speed_4_engaged = false; } }

                            if(((millis() - stat) >= 40000) && (millis() - stat) < 280000){ // for ... 4 minutes
                                if(!fan1_speed_4_engaged){ analogWrite(fan_1, 255); /*ledcWrite(channel_1, Speed_4); /*  Serial.print("Fan 1 Speed: "); Serial.println(Speed_4);  */ fan1_speed_4_engaged = true; fan1_speed_2_engaged = false;} } //to avoid repititions
