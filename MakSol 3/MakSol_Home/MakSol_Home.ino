...
// --- PWM OUTPUTS
    const uint8_t Coil_1= 14; //26;   // 14; //26;  //13; // 14; //33
    const uint8_t Coil_2= 26; //14;  //  26;  
    const uint8_t fan_1 =    33;
    const uint8_t fan_2 =    25;
...
//PORTD = B00001001; // write high bits on those two ports
  ...
  Screen.showNumber(1234567);  delay(100); // overflow     --- 2
    ...
    //STATIC FAN
    pinMode(fan_1, OUTPUT); digitalWrite(fan_1, HIGH);
    pinMode(fan_2, OUTPUT); digitalWrite(fan_2, HIGH);
...
float summation = 0.00;
  ...
  for(int i=0; i<10; i++){ //7 samples
    ...
    //  voltage = 2 * 14.868 * (summation/3095.0);
        voltage = 2 * 13.00 * (summation/3095.0);
...
if(voltage <= 21.0){ //PD: 10.5 (EMF:11.0V) equivalent

     Battery_Level_Counter = 0; 
     if(!isCharging) {digitalWrite(charge_by_Solar, HIGH); isCharging = true; beep(1);} //No Matter how low the Voltage is, CHARGE
    ...
    if(voltage > 20.20 && voltage <= 21.0){ //turn OFF MAIN COOKING ZONE only

             if(heat_2_level != 0){ // === low power alarm
               beep(3);
    ...
    else if(voltage <= 20.20 ) { // turn OFF the smaller Cooking Zone As Well ... 11.0V terminal EMF
            
          if(heat_1_level != 0){ // === low power
            beep(3);
...
else{ //if voltage is  20.20 at the very least
...
//between 10.10V to 11.10V // repsentative of 11.0V to 12.0V
    if(voltage >= 21.00 && voltage < 22.20) {// ONE BAR
...
//between 11.10V to 12.30 // repsentative of 12.0V to 12.55V        
 else if(voltage >= 22.20 && voltage < 24.60) { //TWO BARS
 ...
 else if(voltage >= 24.60) { //THREE BARS at only 12.3V
      ...
      if(voltage < 25.00){ //anything below 12.5V should auto toggle charging relay
             if(!isCharging) {  digitalWrite(charge_by_Solar, HIGH); isCharging = true; beep(1);  } 
      } // do nothing if 12.6 - 14.40V 
      else if(voltage > 30.10){ if(isCharging) { // redundant but required for safety... just in case
...
// Serial.println("Cooker not charging..."); Serial.println();
        ...
        if(voltage < 21.20){ //CRITICAL POWER INDICATOR
                  if(!LED0_ON)  {  digitalWrite(level0, HIGH);  LED0_ON = true; }  
                  if(LED1_ON)  {  digitalWrite(level1, LOW);  LED1_ON = false; } 
                  if(LED2_ON)  {  digitalWrite(level2, LOW);  LED2_ON = false; }
                  if(LED3_ON)  {  digitalWrite(level3, LOW);  LED3_ON = false; }
   ...
   else  if(voltage >= 21.00 && voltage < 22.20){ //only one bar (blinking) till 12.20V
              Battery_Level_Counter = 1;
                  if(LED0_ON)  {  digitalWrite(level0, LOW); LED0_ON = false; } //OFF CRITICAL POWER INDICATOR
                  if(LED2_ON)  {  digitalWrite(level2, LOW);  LED2_ON = false; }
                  if(LED3_ON)  {  digitalWrite(level3, LOW);  LED3_ON = false; }
    ...
    else if(voltage > 22.60 && voltage <= 25.60){ //two bars bar lock in at 12.21V, middle blinking
            Battery_Level_Counter = 2;
    ...
    else if(voltage > 25.60){ //three bars bar lock in at 13.3V .... blink the top most
              Battery_Level_Counter = 3;
                ...
                if(voltage > 29.00)  { // make all bars static: no bar blinks... just like 12.8V for not charging
                           if(LED0_ON)  {  digitalWrite(level0, LOW); LED0_ON = false; } //OFF CRITICAL POWER INDICATOR
                           if(!LED1_ON) {  digitalWrite(level1, HIGH); LED1_ON = true; } //KEEP LOWER BAR LIT
                           if(!LED2_ON) {  digitalWrite(level2, HIGH); LED2_ON = true; } // KEEP MIDDLE BAR LIT
                           if(!LED3_ON) {  digitalWrite(level3, HIGH); LED3_ON = true; } // KEEP THE TOP BAR ON
                  
                     if(voltage > 30.10) {digitalWrite(charge_by_Solar, LOW); isCharging = false; beep(3); } 
                        }// PREVENT OVERCHARGE
          }
...
if(up2 == 1) {  if(heat_2_level < 5) {locked_up2=false; locked_down2=false; //free_run
                    ...
                    Serial.print("Button Pushed: "); Serial.println(up2);
                    }
...
if(coil_1_index != 0) { //if toggled to 1, 2, 3, 4, 5
    ...
    */
      ...
      /*GEAR 1*/          if(!fan1_ON) { // 30 seconds
                              digitalWrite(fan_1, HIGH); fan1_ON = true;  fan1_speed_1_engaged = true; fan1_speed_3_engaged = false; fan1_speed_5_engaged = false; /* Serial.print("Fan 1 Speed: "); Serial.println(Speed_1); */ }
      ...
      /*GEAR 3*/             if(fan_1_timer >= 30000 && fan_1_timer < 90000){ // for ... 1 minute: 60 seconds || till 1:30
                                if(!fan1_speed_3_engaged){digitalWrite(fan_1, HIGH); fan1_speed_3_engaged = true; fan1_speed_1_engaged = false; fan1_speed_5_engaged = false;} } /*  Serial.print("Fan 1 Speed: "); Serial.println(Speed_4);  */

      /*GEAR 5*/             if(fan_1_timer >= 90000 && fan_1_timer < 270000){ // for ... 3minutes: 180 seconds || till 4:30
                                if(!fan1_speed_5_engaged){digitalWrite(fan_1, HIGH);  fan1_speed_5_engaged = true; fan1_speed_3_engaged = false;  fan1_speed_1_engaged = false;} } /*  Serial.print("Fan 1 Speed: "); Serial.println(Speed_4);  */
      ...
      /*GEAR 1*/             if(fan_1_timer >= 330000 && fan_1_timer < 360000){ // for... 60 seconds || till 6:30
                                if(!fan1_speed_1_engaged){digitalWrite(fan_1, HIGH); fan1_speed_1_engaged = true; fan1_speed_3_engaged = false; fan1_speed_5_engaged = false; } }  /* Serial.print("Fan 1 Speed: "); Serial.println(Speed_2);  */ 

      /*GEAR 3*/             if(fan_1_timer >= 360000 && fan_1_timer < 450000){ // for... 90 seconds || till 7:30
                                if(!fan1_speed_3_engaged){digitalWrite(fan_1, HIGH); fan1_speed_3_engaged = true; fan1_speed_1_engaged = false; fan1_speed_5_engaged = false;} }  /* Serial.print("Fan 1 Speed: "); Serial.println(Speed_2);  */ 

      /*GEAR 5*/             if(fan_1_timer >= 450000){ //after 7.5 minutes, keep fan running at full blast || till inf
                                if(!fan1_speed_5_engaged){digitalWrite(fan_1, HIGH); fan1_speed_5_engaged = true; fan1_speed_3_engaged = false; fan1_speed_1_engaged = false; } }  //to avoid repititions /* Serial.print("Fan 1 Speed: "); Serial.println(Speed_2);  */ 
...
if(coil_2_index != 0) {  //if toggled to a non zero integer
      ...
      /*GEAR 1*/          if(!fan2_ON) {   /*Serial.print(F("Fan 2 Speed: ")); /*Serial.println(F(Speed_1));*/ //
                                        digitalWrite(fan_2, HIGH);  fan2_ON = true;  fan2_speed_1_engaged = true; fan2_speed_2_engaged = false; fan2_speed_3_engaged = false; fan2_speed_4_engaged = false; fan2_speed_5_engaged = false; // Serial.print("Fan 2 Speed: "); Serial.println(Speed_1);  
                                        }
      ...
      /*GEAR 2*/             if(fan_2_timer >= 60000 && fan_2_timer < 75000){ // for ... 15 seconds || till 1:15
                                if(!fan2_speed_2_engaged) {digitalWrite(fan_2, HIGH);  fan2_speed_2_engaged = true;  fan2_speed_1_engaged = false; fan2_speed_3_engaged = false; fan2_speed_4_engaged = false; fan2_speed_5_engaged = false;;   /*  Serial.print("Fan 2 Speed: "); Serial.println(Speed_2); */   } } 

      /*GEAR 3*/             if(fan_2_timer >= 75000 && fan_2_timer < 90000){ // for ... 15 seconds || till 1:30
                                if(!fan2_speed_3_engaged){digitalWrite(fan_2, HIGH);   fan2_speed_3_engaged = true; fan2_speed_1_engaged = false; fan2_speed_2_engaged = false; fan2_speed_4_engaged = false; fan2_speed_5_engaged = false;;   /*  Serial.print("Fan 2 Speed: "); Serial.println(Speed_3); */  } }  

      /*GEAR 4*/             if(fan_2_timer >= 90000 && fan_2_timer < 180000){ // for ... 90 seconds || till 2:30
                                if(!fan2_speed_4_engaged){digitalWrite(fan_2, HIGH);  fan2_speed_4_engaged = true; fan2_speed_1_engaged = false; fan2_speed_2_engaged = false; fan2_speed_3_engaged = false; fan2_speed_5_engaged = false;;    /*  Serial.print("Fan 2 Speed: "); Serial.println(Speed_4); */ } } //MACHINE STATES to avoid repititions 

      /*GEAR 5*/             if(fan_2_timer >= 180000 && fan_2_timer < 390000){ // for ... 210 seconds || till 6:00
                                if(!fan2_speed_5_engaged){digitalWrite(fan_2, HIGH);   fan2_speed_5_engaged = true; fan2_speed_1_engaged = false; fan2_speed_2_engaged = false; fan2_speed_3_engaged = false; fan2_speed_4_engaged = false;;    /*  Serial.print("Fan 2 Speed: "); Serial.println(Speed_5); */ } } 
      ...
      /*GEAR 1*/             if(fan_2_timer >= 450000 && fan_2_timer < 480000){ // for... 30 seconds || till 8:00
                                if(!fan2_speed_1_engaged){digitalWrite(fan_2, HIGH);  fan2_speed_1_engaged = true; fan2_speed_2_engaged = false; fan2_speed_3_engaged = false; fan2_speed_4_engaged = false; fan2_speed_5_engaged = false;; /* Serial.print("Fan 2 Speed: "); Serial.println(Speed_1);*/} }  
