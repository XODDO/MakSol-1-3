...
// --- TOUCH INPUTS
 ...
 const int powerBtn = 17; //const int powerLED = 34;
...
//ATTACK THE COILS first --- use direct port access to lower response to nanosex 
  pinMode(Coil_1, OUTPUT);    pinMode(Coil_2, OUTPUT);  // ACTIVE HIGH

  digitalWrite(Coil_1, HIGH);  digitalWrite(Coil_2, HIGH);  // ACTIVE LOW
...
while(!activated){
  ...
  if(tiktok == 10) digitalWrite(PowerLED, HIGH);
  if(tiktok == 50000) digitalWrite(PowerLED, LOW);
  if(tiktok == 1e6) tiktok = 0;

    //Serial.println(tiktok);

       detectPress = digitalRead(powerBtn);
    ...
    if(firstPass == true){
        ...
        if((millis() - readStart) > 800) activated = true; 
    } 
}

digitalWrite(PowerLED, LOW);

  pinMode(fan_1, OUTPUT);     pinMode(fan_2, OUTPUT); 
  ...
  while(foo>=0) { delay(75);
    ...
    //beep(3);
    uint8_t hoot = 0;
 do{ digitalWrite(sound, HIGH); delay(50); digitalWrite(sound, LOW); delay(50); hoot++; } while(hoot < 2);
...
uint8_t Battery_Level_Counter; bool tasks_killed = false;
//float voltage = 0.00;
...
void batteryLevel(){
 ...
 //Battery_Level_Counter = 0;
  ...
  Serial.println();
  //Serial.print("Raw Reading: "); Serial.println(summation);
  //if(summation < 2000) {
  voltage = 15.00 * float(summation)/3095.0;
  Serial.print("Battery: "); Serial.println(voltage, 5);
/*
  else {
     voltage = 14.00 * float(summation)/3095.0;
  Serial.print("Battery: "); Serial.println(voltage, 5);
  }
*/
  if(voltage < 10.80) { //too low battery power --- 11.00V
     Battery_Level_Counter = 0;
    ...
    if(!isCharging) { digitalWrite(SolarCharge, HIGH); isCharging = true; Serial.println("Charge Engaged!");} // ALWAYS PREVENT DEEP DISCHARGE when there is sunshining...
 
  }
  else if(voltage >= 10.80 && voltage < 11.55) {Battery_Level_Counter = 1; isPower_enough = true; //end_processes = false;
        if(!LED1_ON) {digitalWrite(level1, HIGH); LED1_ON = true; }
        if(LED2_ON)  {digitalWrite(level2, LOW); LED2_ON = false; } 
        if(LED3_ON)  {digitalWrite(level3, LOW); LED3_ON = false; }
        
        if(!isCharging) { digitalWrite(SolarCharge, HIGH); isCharging = true; Serial.println("Charge Engaged!");} //PWM THE CHARGING VOLTAGE
        
     if(voltage == 10.80) {// kill all RUNNING TASKS... store in EEPROM and reconnect at 12.5V
        if(!tasks_killed){
        EEPROM.write(10, 0);
        EEPROM.commit();
        tasks_killed = true;
  ...
  if(voltage >= 11.55 && voltage < 12.70) {Battery_Level_Counter = 2; isPower_enough = true;
      if(!LED2_ON) {digitalWrite(level2, HIGH); LED2_ON = true; } //LOAD RECONNECT SEQRNCE AT 12.8 --- CHARGING VOLTAGE
      if(!LED1_ON) {digitalWrite(level1, HIGH); LED1_ON = true; }
      if(LED3_ON)  {digitalWrite(level3, LOW);  LED3_ON = false;}
      
      if(voltage <= 12.50) if(!isCharging) { digitalWrite(SolarCharge, HIGH); isCharging = true; Serial.println("Charge Engaged!");}} //pulse width modulate the charging voltage
      else { 
        if(tasks_killed){ //reconnect from critical power
             EEPROM.write(10, 1);
             EEPROM.commit();
             tasks_killed = false;
          }
  ...
  if(voltage >= 12.70) {Battery_Level_Counter = 3; isPower_enough = true;
      ...
      if(voltage >= 14.30) if(isCharging) { digitalWrite(SolarCharge, LOW); isCharging = false; Serial.println("Charge Dis-engaged!");} // PREVENT OVERCHARGE
      }
...
const   long wait_ko = 30000; // 30 sec ideal Cooling time \\ 300,000

const   long interval_1 = 120000;  //  2 minutes
const   long interval_2 = 600000;  // 10 minutes
const   long interval_3 = 1200000; // 20 minutes
const   long interval_4 = 1800000; // 30 minutes
const   long interval_5 = 3000000; // 50 minutes
const   long interval_6 = 5400000; // 90 minutes

unsigned int idle = 0; //if it stays too long idling yet toggled to cook, alarm and power down coil
unsigned int prob_battery = 0;
void loop() { prob_battery++; 
/* ONLY 3 TASKS:
 * Read battery level once every 1000 counts
 * Listen to touch interrupts
 * Update cooking coils manager
 * Shutdown when pressed or when idle for 20 minutes 
 */

//if(prob_battery==100) { prob_battery=0;
batteryLevel(); //}
touchReader();
...
void listen_to_shutdown(){ //go to deep sleep either on Power Shutdown or idling for 20 minutes
       //int readBtn = digitalRead(power);
       //     if(readBtn) kill all running processes
...
unsigned int flashing = 0; bool blinking = false;
...
void touchProcessor(){ // LEVEL SHIFTER WITH SEMI-DYNAMIC POWER MANAGEMENT INTEGRATED
   if(Battery_Level_Counter < 1) { //only react to LEVEL LOWERING when nearing critical Power 
        if(down1 == 1) {  //allow reductions
        ...
        if(down2 == 1) { //alow reductions
            ...
            if(up1 || up2) beep(3); // reject additions
...
if(up1 == 1 || up2 == 1 || down1 == 1 || down2 == 1){ flashing = 0; digitalWrite(PowerLED, HIGH); beep(1); digitalWrite(PowerLED, LOW);}
...
unsigned long started_beeping = 0;
...
//redundace to be removed
/*
* 3 styles to do this: 
1 blocking code nested into a for loop
2 using millis line by line
3 using millis in a recursive function: (n, quitting at 100n-50)
*/
/*
      if(!sound_started) { started_beeping = millis(); digitalWrite(sound, HIGH);  sound_started = true;  }
      if (sound_started) {
        
        if(times <= 1){ //OFF in 1
          if((millis() - started_beeping) >= 50) { 
           digitalWrite(sound, LOW); sound_started = false;
          }
        }
        if(times >= 2) { //OFF in 2
          if((millis() - started_beeping) >= 50 && (millis() - started_beeping) < 100) { 
            digitalWrite(sound, LOW); 
          }

          if((millis() - started_beeping) >= 100 && (millis() - started_beeping) < 150) { //ogwokubili
            digitalWrite(sound, HIGH); 
          }

         if(times == 2){
          if((millis() - started_beeping) >= 150) { // pack bag and leave
            digitalWrite(sound, LOW);  sound_started = false;
          }
         }

          if(times == 3){ //OFF in 3
            if((millis() - started_beeping) >= 150 && (millis() - started_beeping) < 200) { 
              digitalWrite(sound, LOW);  
            }
            if((millis() - started_beeping) >= 200 && (millis() - started_beeping) < 250) { //ogwokusatu
              digitalWrite(sound, HIGH); 
            }
            if((millis() - started_beeping) >= 250) { 
              digitalWrite(sound, LOW); sound_started = false;
            }
          ...
          if(times == 4){ //OFF in 4
            ...
            if((millis() - started_beeping) >= 200 && (millis() - started_beeping) < 250) { //ogw'okusatu
              digitalWrite(sound, HIGH); 
            }
            if((millis() - started_beeping) >= 250 && (millis() - started_beeping) < 300) { 
              digitalWrite(sound, LOW); 
            }
            if((millis() - started_beeping) >= 300 && (millis() - started_beeping) < 350) { //ogw'okuna
              digitalWrite(sound, HIGH); 
            }
            if((millis() - started_beeping) >= 350) { 
              digitalWrite(sound, LOW); sound_started = false;
            }
...
/*
// if battery was found in a crisis
        if(tasks_killed){
           beep(3); return; //to avoid the GIANT else
         }
*/

if(Coil_1_ON || Coil_2_ON){ // 1 second blinker
        flashing++;
     if(flashing == 50) {  if(!blinking){digitalWrite(PowerLED, 1); blinking = true;} }
else if(flashing >= 150){  if(blinking) {digitalWrite(PowerLED, 0); blinking = false;} 
     if(flashing >= 200)  flashing = 0;
    }    
      Serial.print("\t\t\tBlinker: "); Serial.println(flashing);
}
else  {if(blinking) { digitalWrite(PowerLED, 0); blinking = false;}} //if(!Coil_1_ON && !Coil_2_ON)


/*CRITICAL POWER MODE*/
if(Battery_Level_Counter < 1) { //TURN OFF ALL HIGH POWER CONSUMERS if POWER IS ALARMINGLY LOW: <10.9V
  if(Coil_1_ON) { heat_1_level = 0; Screen.display(0, heat_1_level);
                  digitalWrite(fan_1, LOW); //switch OFF
                  digitalWrite(Coil_1, HIGH); Coil_1_ON = false; waiting = false;
                  stat = 0; Serial.println("Turned OFF"); }
  if(Coil_2_ON) { heat_2_level = 0; Screen.display(0, heat_2_level);
                  digitalWrite(fan_2, LOW); //switch OFF
...
/*LOW POWER MODE*/
else if(Battery_Level_Counter == 1) {  //Automatically LOWER CONSUMPTION RATE TO ONE COIL when 10.80 to 11.55V

if(Coil_1_ON){
    if(heat_1_level>1){ heat_1_level = 1; Screen.display(0, heat_1_level); 
    }
  }

  if(Coil_2_ON){
      if(heat_2_level>1){ heat_2_level = 1; Screen.display(3, heat_2_level); 
    }
  }

}
// else if(Battery_Level_Counter == 2) // SLOWLY LOWER heat_1/2_level TO 3 MAX

else { // allow freedom when battery level is 12V and above
...
//close if zero
  ...
  switch(coil_1_index){ 
      case 1: interval = interval_1; running(interval); Serial.print("Running Time (min): "); Serial.println(interval/60000);  
            break;

      case 2: interval = interval_2; running(interval); Serial.print("Running Time (min): "); Serial.println(interval/60000); 
              break;

      case 3: interval = interval_3; running(interval); Serial.print("Running Time (min): "); Serial.println(interval/60000); 
              break;

      case 4: interval = interval_4; running(interval); Serial.print("Running Time (min): "); Serial.println(interval/60000); 
              break;
      
      case 5: interval = interval_5; running(interval); Serial.print("Running Time (min): "); Serial.println(interval/60000); 
              break;
...
//close if zero
  ...
  switch(coil_2_index){ 
      case 1: interval2 = interval_1; running(interval2); Serial.print("Running Time (min): "); Serial.println(interval2/60000);  
            break;

      case 2: interval2 = interval_2; running(interval2); Serial.print("Running Time (min): "); Serial.println(interval2/60000); 
              break;

      case 3: interval2 = interval_3; running(interval2); Serial.print("Running Time (min): "); Serial.println(interval2/60000); 
              break;

      case 4: interval2 = interval_4; running(interval2);  Serial.print("Running Time (min): "); Serial.println(interval2/60000); 
              break;
      
      case 5: interval2 = interval_5; running(interval2);  Serial.print("Running Time (min): "); Serial.println(interval2/60000); 
              break;
