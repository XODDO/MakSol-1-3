...
// --- TOUCH INPUTS
 ...
 const int power = 17; //const int powerLED = 34;
...
//OFF
if(power_toggled) {digitalWrite(LED2, LOW); power_toggled = false;}
...
while(!activated){
       ...
       else {arrested = false; readStart = millis(); digitalWrite(LED2, LOW); }

    if(arrested && !firstPass){readStart = millis(); firstPass = true;}
    if(firstPass == true){
      if(!power_toggled){digitalWrite(LED2, HIGH); power_toggled = true;}
  ...
  while(foo>=0) { delay(75);
    ...
    digitalWrite(LED2, LOW); power_toggled = false;
...
uint16_t reading;
  ...
  voltage = 15.00 * float(summation)/3090.0;
...
if(up1 == 1 || up2 == 1 || down1 == 1 || down2 == 1){ digitalWrite(LED2, HIGH); beep(1); digitalWrite(LED2, LOW);}
