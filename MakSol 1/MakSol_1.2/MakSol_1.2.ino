/* 18 DEC 2022 EDITION ~650lines
Deployed in the Mbale MakSol cooker... 
  ...
  * BASED ON 4-DIGIT TM1637 7seg DISP, uses pnly 2 digits
*/
#include "Arduino.h"
...
// --- UNIVERSAL INDICATORs
 const int LED = 2; //POWER LED
 const int LED2  = 21; //23
 const int sound = 32;
 const int is_solaring = 0;


const int coil_1_led = 15;
const int coil_2_led = 2;

// --- ANALOG INPUTS
 const int readSolar = 36;  //  26;
 const int readBattery = 39;
 const int readCurrent = 32;  
  
 //const int coil_1_temp = 34;  
 //const int coil_2_temp = 35;  
 //const int readCurrent = 32;  
...
// --- PWM OUTPUTS
  ...
  const int SolarCharge = 12; //14; //17-Dec-2022

// --- TOUCH INPUTS
 const int touch_1 = 4;   // up 1
 const int touch_2 = 23; //27; //15; // down 1
 const int touch_3 = 5; //13; // up 2
 const int touch_4 = 22; //16;// down 2
...
uint16_t reading;
  ...
  if(voltage < 11.10) { //too low battery power
     Battery_Level_Counter = 0;
...
if(heat_1_level > 0) { heat_1_level = 0; Monitor(heat_1_level, heat_2_level); locked_down = true; locked_up=false; Screen.display(0, heat_1_level); } 
if(heat_2_level > 0) { heat_2_level = 0; Monitor(heat_1_level, heat_2_level); locked_down2 = true; locked_up2=false; Screen.display(3, heat_2_level);}
  

            
  }
  else if(voltage >= 11.10 && voltage < 11.90) {Battery_Level_Counter = 1; isPower_enough = true; end_processes = false;
 ...
 else if(voltage >= 11.90 && voltage < 12.55) {Battery_Level_Counter = 2; isPower_enough = true;
      ...
      if(voltage >= 14.45) if(isCharging) { digitalWrite(SolarCharge, LOW); isCharging = false; beep(3);} // PREVENT OVERCHARGE
      }
...
if(up1 || up2 || down1 || down2){ lock_press = true; off_triggered = false; digitalWrite(LED2, HIGH); beep(1); digitalWrite(LED2, LOW); } 
...
uint8_t off_touched = 0;
unsigned long quit_counter = 0; bool fast_pass = false;

void listen_to_shutdown(){ //go to deep sleep

  off_touched = digitalRead(power);
/**/
if(off_touched) fast_pass = true;
else quit_counter = millis();

if(fast_pass && !off_triggered){ //beep(1, false);
       off_triggered = true; quit_counter = millis();
       Serial.println(); Serial.println("Shutting Down!"); Serial.println();
}

  if(off_triggered){
     if((millis() - quit_counter)>= 200){ //if press duration = 1/10th of a sec
            fast_pass = false;
                    if(Coil_2_ON) {
                                    while(heat_2_level > 0){
                                        heat_2_level--;
                                        beep(1);
                                        delay(100);
                                        Screen.display(3, heat_2_level);
                      ...
                      locked_up2=false;
                      Serial.println("COIL 2 Turned OFF DUE TO MANUAL SHUTWOWN");
                    }

                    if(Coil_1_ON) {
                                    while(heat_1_level > 0){
                                        heat_1_level--;
                                        beep(1);
                                        delay(100);
                                        Screen.display(0, heat_1_level);
                                    }
                    locked_up=false; 
                    Serial.println("COIL 1 Turned OFF DUE TO MANUAL SHUTWOWN");
                    }            
              }
...
void Monitor(uint8_t coil_1_index, uint8_t coil_2_index){ //COIL INDEX IS THE MASTER CONTROLLER 
  ...
  //kozesa coil 1 index otambuze coil 1 state ... and likewise 2 4 2
      if(coil_1_index == 0){  //switch OFF // if either we haven't started or they have reduced all the way to zero
        if(waiting) {digitalWrite(fan_1, LOW); waiting = false;}
              ...
              if(Coil_1_ON) { digitalWrite(fan_1, LOW);  waiting = false; 
                              digitalWrite(Coil_1, HIGH); Coil_1_ON = false;
