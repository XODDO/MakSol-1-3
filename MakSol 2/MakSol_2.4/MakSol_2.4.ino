/* 4th APRIL 2023 EDITION ~650lines
...
float summation = 0.00;
  ...
  for(int i=0; i<10; i++){ //7 samples
    ...
    //voltage = 14.50 * (summation/3095.0);
      voltage = 14.75 * (summation/3095.0);
...
*/
  ...
  //   isPower_enough = false; 
     ...
     if(heat_1_level > 0) { // display for the left most cooking zone                      
                            for(int index = heat_1_level; index>=0; index--){ Screen.showNumber(index, false, 1, 0);  delay(50); }
                            heat_1_level = 0;  
                            Monitor_cookingZones(heat_1_level, heat_2_level);
 ...
 int workin_volt = voltage*100; char voltage_str2[5];
      ...
      if(lock_press && !passed_yet) {start_counting = millis(); passed_yet = true;}
        if(passed_yet){
         ...
         /*EXTRA SETTINGS MODE*/    //e.g. TIMER, VOLTAGE DISP, 
          if((millis() - start_counting) >= 1500) { beep(3); }  
...
if(Battery_Level_Counter < 1) { //if POWER IS ALARMINGLY LOW < 11.4V auto stop cooking
      if(coil_1_index == 0) { 
                    ...
                    if(Coil_1_ON) {
                      digitalWrite(fan_1, LOW); waiting = false; fan1_ON = false;
      ...
      if(coil_2_index > 0) { // display for the left most cooking zone                      
              //             for(int index = coil_2_index; index>=0; index--){ Screen.showNumber(index, false); delay(100); }  //Screen.showNumber(coil_1_index, false, 1, 0);
                              
                 //         coil_2_index = 0; Serial.print("Tuned to "); Serial.println(coil_2_index);
                        if(Coil_2_ON) {
                            digitalWrite(fan_2, LOW);  waiting2 = false; fan2_ON = false; Serial.println("Fan 2 OFF!");
                            digitalWrite(Coil_2, HIGH); Coil_2_ON = false;
                            Serial.println("COIL 2 Turned OFF due to POWER"); 
                            stat2 = 0;  
                        }
                    }

        if(waiting)  {if(fan1_ON) {digitalWrite(fan_1, LOW); waiting  = false; fan1_ON = false;}}
        if(waiting2) {if(fan2_ON) {digitalWrite(fan_2, LOW); waiting2 = false; fan2_ON = false; Serial.println("Fan 2 OFF!");}}


//TURN BACK THE DISPLAYS to ZERO
/*
    ...
    if(heat_1_level > 0) { heat_1_level = 0; Monitor_cookingZones(heat_1_level, heat_2_level); locked_down = true; locked_up=false; 
                           Screen.showNumber(heat_1_level, false, 1, 0); digitalWrite(coil_1_LED, LOW);} 
  ...
  //kozesa coil 1 index otambuze coil 1 state ... and likewise 2 4 2
      if(coil_1_index == 0){  //switch OFF // if either we haven't started or they have reduced all the way to zero
        if(waiting) {if(fan1_ON){digitalWrite(fan_1, LOW); waiting = false; fan1_ON = false;}}
              ...
              if(Coil_1_ON) { digitalWrite(fan_1, LOW);  waiting = false; fan1_ON = false;
                              digitalWrite(Coil_1, HIGH); Coil_1_ON = false;
...
//close if power is set to zero
     ...
     if(Coil_1_ON) { 
        ...
        if(!waiting){//throw the fan 2 seconds out of start phase
                     if(!fan1_ON) if((millis() - stat) > 5000) { digitalWrite(fan_1, HIGH); fan1_ON = true;}
                    }
       }
...
if(coil_2_index == 0){ //switch OFF // Serial.println("Coil 2 OFF!"); // if they have reduced to zero
   if(Coil_2_ON) {digitalWrite(fan_2, LOW); waiting2 = false; fan2_ON = false; Serial.println("Fan 2 OFF!");
                  digitalWrite(Coil_2, HIGH); Coil_2_ON = false; 
                  stat2 = 0;/*/ Serial.println("Turned OFF"); */}// 
    if(waiting2) {digitalWrite(fan_2, LOW); waiting2 = false; fan2_ON = false; Serial.println("Fan 2 OFF!");}
    }
//close if zero
      ...
      if(!Coil_2_ON) { //check if it hasn't alreay been turned ON
          if(!waiting2) { //turn it ON if it is not in waiting state
            ...
            if(waiting2){
              ...
              //throw the fan 3-5 seconds out of start phase and make it run intermittently e.g. 1min ON, 1min OFF
                        if(!fan2_ON) if((millis() - stat2) >= 3000) { digitalWrite(fan_2, HIGH); fan2_ON = true; Serial.println("Fan 2 ON!");}
                        }
          }
          else if(Coil_2_ON) { 
        //    Serial.print("Coil "); Serial.print(CoilNum); Serial.print(" running for: "); 
      //     Serial.println((1000 + stat2 + interval2 - millis())/1000);
                if(!waiting2){//throw the fan 2 seconds out of start phase
                              if(!fan2_ON) if((millis() - stat2) > 3000) { digitalWrite(fan_2, HIGH); fan2_ON = true; Serial.println("Fan 2 ON!");}
                            }
          }
   ...
   // Serial.print("Running Time: "); Serial.println(intervo); 
              ...
              if((millis() - stat) < interval) { //Serial.print("Coil "); Serial.print(CoilNum); Serial.println(" ON");
                    /*Serial.print(" Firing till ");  Serial.println((1+interval + stat)/1000);*/  /*if(reducing) waiting = true;*/
                      ...
                      else if(blink_counta > 300 && blink_counta <= 800) { if(is_togod){digitalWrite(flashin_LED, LOW); is_togod = false;} }
