...
// --- PWM OUTPUTS
  ...
  const int fan_1 = 12;
  const int fan_2 = 27;
  const int SolarCharge = 26; //14; //17-Dec-2022
...
*/
  ...
  while(foo>=0) { delay(75);
        ...
        Screen.showNumber(foo, false, 1, 2); // the left most digit
             foo--;
     }
...
uint16_t reading;
  ...
  if(voltage < 11.30) { //too low battery power
     Battery_Level_Counter = 0;
...
*/
    ...
    if(heat_1_level > 0) { heat_1_level = 0; Monitor(heat_1_level, heat_2_level); locked_down = true; locked_up=false; 
                           Screen.showNumber(heat_1_level, false, 1, 2); } 
                          
    // the left most cooking zone                      
    if(heat_2_level > 0) { heat_2_level = 0; Monitor(heat_1_level, heat_2_level); locked_down2 = true; locked_up2=false; 
                        Screen.showNumber(heat_2_level, false); Screen.showNumber(heat_1_level, false, 1, 2);}
  
 }
  
  else if(voltage >= 11.30 && voltage < 11.90) {Battery_Level_Counter = 1; isPower_enough = true; end_processes = false;
      ...
      if(voltage >= 14.25) if(isCharging) { digitalWrite(SolarCharge, LOW); isCharging = false; beep(3);} // PREVENT OVERCHARGE
      }
   ...
   if(Battery_Level_Counter < 1) { //only react to LEVEL LOWERING when in ultra low power mode
        ...
        if(down1 == 1) {  
              if(heat_1_level > 0) {locked_down=false; locked_up=false; heat_1_level--; } 
                    ...
                    Screen.showNumber(heat_1_level, false, 1, 2); reducing = true; }
    ...
    if(up1 == 1) {  
                 ...
                 Screen.showNumber(heat_1_level, false, 1, 2); CoilNum = 1; reducing = false; 
               }
...
if(up2 == 1) {  if(heat_2_level < 5) {locked_up2=false; locked_down2=false; //free_run
                    ...
                    Screen.showNumber(heat_2_level, false); Screen.showNumber(heat_1_level, false, 1, 2); CoilNum = 2;  }
