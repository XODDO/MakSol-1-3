/* 30th AUGUST 2023 EDITION ~900 lines: ~50% more from 650lines
  ...
  * HIGH PRECISION SCHEDULING => using (millis()) as time allocator
  * GOING TO USE int64_t esp_timer_get_time(void); SINCE IT OVERFLOWS AT 200k YEARS
...
unsigned long listens_to_user = 0;
...
*/
  ...
  if(!Coil_1_ON && !Coil_2_ON) {if(batt_runs%80000 == 0) MonitorBattery();  // 100k loop cycles for each second at 240MHz 
                                ...
                                if(batt_runs%20000 == 0) MonitorBattery(); // Query the battery once every second since it doesn't change that rapidly
    }
...
//10 hours
 //if((millis() - time_of_last_activity) >= ) esp_light_sleep_start(); //Mode: The CPU is paused. Any wake-up events (MAC, host, RTC timer, or external interrupts) will wake up the chip.
 //if(wake_up_call){}
...
unsigned long coil_1_started = 0, coil_2_started = 0; 
...
unsigned long auto_turn_off = 18000000; // 5 hours //18 million milliseconds

unsigned long runtime_tracker = 0;
unsigned long fan_1_timer = 0;
unsigned long fan_2_timer = 0;
 ...
 //WHENEVER POWER IS ALARMINGLY LOW < 11.4V auto stop cooking
      if(coil_1_index == 0) { 
                    if(Coil_1_ON) {
                        ...
                        coil_1_started = 0;
                        fan_1_stop = millis(); //fan_2_stop = now;
                      }
                    if(fan1_ON){ //whether it was in the wait or NOT, take it OFF after 5 seconds
                          if(millis() - fan_1_stop >= 5000) { // forced cooling at shutdown
      ...
      if(coil_2_index == 0) { 
                        if(Coil_2_ON) {
                            ...
                            coil_2_started = 0;  
                            fan_2_stop = millis(); // fan_2_stop = now;  
                        }
                      if(fan2_ON){ //whether it was in the wait or NOT, take it OFF after 5 seconds
                          if(millis() - fan_2_stop >= 5000) { // ((now - fan_2_stop) >= 4000) forced cooling at shutdown
...
if(coil_1_index != 0) { //if toggled to 1, 2, 3, 4, 5
    ...
    */
      ...
      /*GEAR 1*/          if(!fan1_ON) { // 30 seconds
                          ...
                          if(fan1_ON)  { fan_1_timer = (long)(millis() - coil_1_started); // = (now - coil_1_started);
      ...
      /*GEAR 1*/             if(fan_1_timer >= 330000 && fan_1_timer < 390000){ // for... 60 seconds || till 6:30
      ...
      /*GEAR 3*/             if(fan_1_timer >= 390000 && fan_1_timer < 450000){ // for... 60 seconds || till 7:30
      ...
      /*GEAR 5*/             if(fan_1_timer >= 450000){ //after 7.5 minutes, keep fan running at full blast || till inf
            ...
            if(!waiting) { //turn it ON if it is not in waiting state
                  ...
                  digitalWrite(Coil_1, LOW); Coil_1_ON = true; coil_1_started = millis(); //coil_1_started = now;
...
if(coil_2_index != 0) {  //if toggled to a non zero integer
      ...
      /*GEAR 1*/          if(!fan2_ON) {   /*Serial.print(F("Fan 2 Speed: ")); /*Serial.println(F(Speed_1));*/ //
                          ...
                          if(fan2_ON){ //only one JAB
                                fan_2_timer = (long)(millis() - coil_2_started); // fan_2_timer = now - coil_2_started;

      /*GEAR 2*/             if(fan_2_timer >= 60000 && fan_2_timer < 75000){ // for ... 15 seconds || till 1:30
      ...
      /*GEAR 3*/             if(fan_2_timer >= 75000 && fan_2_timer < 90000){ // for ... 15 seconds || till 2:00
      ...
      /*GEAR 5*/             if(fan_2_timer >= 510000){ //after 9 minutes, keep fan running at full blast || beyond 8:30
          ...
          if(!Coil_2_ON) { 
            ...
            if(waiting2){ //thrown into WAIT LOOP
                  ...
                  digitalWrite(Coil_2, LOW); Coil_2_ON = true; coil_2_started = millis(); // coil_2_started = now;
...
void running(long intervo){ // uint8_t COIL_NO
   ...
   // Serial.print("Running Time: "); Serial.println(intervo); 
              ...
              if((millis() - coil_1_started) < interval) {//((now - coil_1_started) < interval) //Serial.print("Coil "); Serial.print(CoilNum); Serial.println(" ON");
                    /*Serial.print(" Firing till ");  Serial.println((1+interval + coil_1_started)/1000);*/  /*if(reducing) waiting = true;*/
                           if(blink_counta <= 5000){ if(!is_togod) { digitalWrite(flashin_LED, HIGH); is_togod = true;}}
                      else if(blink_counta > 5000 && blink_counta <= 15000) { if(is_togod){digitalWrite(flashin_LED, LOW); is_togod = false;} }
                  ...
                  if((millis() - coil_1_started) < (interval + wait_ko)){ //now - coil_1_started // --- TURN OFF & induce internal FORCED COOLING
              ...
              if(millis() - coil_2_started < interval2) { // (now - coil_2_started < interval2) /*Serial.print("Coil "); Serial.print(CoilNum); Serial.println(" ON"); */
                          if(blink_counta <= 5000){ if(!is_togod) { digitalWrite(flashin_LED, HIGH); is_togod = true;}}
                      else if(blink_counta > 5000 && blink_counta <= 15000) { if(is_togod){digitalWrite(flashin_LED, LOW); is_togod = false;} }
                      else if(blink_counta > 15000) {blink_counta = 0; is_togod = false; }
                  ...
                  if(millis() - coil_2_started < (interval2 + wait_ko)){// now - coil_2_started // --- TURN OFF & induce internal FORCED COOLING
