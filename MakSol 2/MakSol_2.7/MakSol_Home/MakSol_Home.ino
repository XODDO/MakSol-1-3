/* 30th AUGUST 2023 EDITION ~900 lines: ~50% more from 650lines
...
//unsigned long batt_tap = 0, stop_reading = 0;
//batt_tap = micros();
...
*/
  ...
  if(!Coil_1_ON && !Coil_2_ON) {if(batt_runs%100000 == 0) MonitorBattery();  // Query the battery once every second since it doesn't change that rapidly
      }
  else { //if either coil 1 is on or 2 is ON or BOTH
                                if(batt_runs%50000 == 0) MonitorBattery();
    }
...
if(coil_2_index != 0) {  //if toggled to a non zero integer
      ...
      if(Coil_2_ON) { //check if it hasn't alreay been turned ON
            ...
            if(!waiting2){//turn it on instantly but very silently... for the first FULL minute
                          if(!fan2_ON) {   /*Serial.print(F("Fan 2 Speed: ")); /*Serial.println(F(Speed_1));*/ //
                                    ledcWrite(channel_2, Speed_1); fan2_ON = true; }
