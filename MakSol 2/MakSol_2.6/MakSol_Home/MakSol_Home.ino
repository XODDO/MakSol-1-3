/* 30th AUGUST 2023 EDITION ~650lines
...
//  SPEED CONTROLLED FANS
 ...
 //    REG_WRITE (GPIO_OUT_W1TS_REG, BIT2); //GPIO2 HIGH (set)
   ...
   // Serial.println(F("Boot Complete!")); //0n
...
*/
  listen_to_TouchCtrl(); // listen for user control 10X every second

  if(!Coil_1_ON && !Coil_2_ON) {if(batt_runs%80000 == 0) MonitorBattery();  // Query the battery once every second since it doesn't change that rapidly
      }
  else { //if either coil 1 is on or 2 is ON or BOTH
                                if(batt_runs%40000 == 0) MonitorBattery();
    }
...
*/
  ...
  if(voltage < 10.10) { //when a Power CCT PD of 10.10V represents 11.0V terminal EMF
     Battery_Level_Counter = 0;
  //   isPower_enough = false; 
     ...
     if(heat_1_level > 0) { // display for the left most cooking zone                      
      ...
      if(heat_2_level > 0) { // display for the left most cooking zone                      
                            for(int index = heat_2_level; index>=0; index--){ Screen.showNumber(index, false);  Screen.showNumber(heat_1_level, false, 1, 0); delay(100); }
                            heat_2_level = 0;
...
else{ //if voltage is  10.10 at the very least
...
//between 10.10V to 11.10V // repsentative of 11.0V to 12.0V
    if(voltage >= 10.10 && voltage < 11.10) {Battery_Level_Counter = 1; isPower_enough = true; processes_ended = false;
        if(LED2_ON)  {  digitalWrite(level2, LOW); LED2_ON = false; } 
        if(LED3_ON)  {  digitalWrite(level3, LOW); LED3_ON = false; }
...
//between 11.10V to 12.30 // repsentative of 12.0V to 12.55V        
          ...
          Battery_Level_Counter = 2; isPower_enough = true;
      ...
      if(LED3_ON)  {  digitalWrite(level3, LOW);  LED3_ON = false;}
       ...
       if(!isCharging) { digitalWrite(charge_by_Solar, HIGH); isCharging = true; beep(1); }
          ...
          Battery_Level_Counter = 3; isPower_enough = true;
...
Serial.println("Is not charging..."); Serial.println();
         ...
         if(voltage >= 10.10 && voltage <= 12.20){ //only one bar (blinking) till 12.20V
                  if(LED0_ON)  {  digitalWrite(level0, LOW); LED0_ON = false; } //OFF CRITICAL POWER INDICATOR
                  if(LED2_ON)  {  digitalWrite(level2, LOW);  LED2_ON = false; }
                  if(LED3_ON)  {  digitalWrite(level3, LOW);  LED3_ON = false; }
                  ...
                  if(blinking_battery)  {  digitalWrite(level1, LOW);  LED1_ON = false;}
                    ...
                    if(voltage >= 14.40)  { // make all bars static: no bar blinks... just like 12.8V for not charging
                            ...
                            if(isCharging) { digitalWrite(charge_by_Solar, LOW); isCharging = false; beep(3); } 
                        }// PREVENT OVERCHARGE
        }
...
//blink power button when charginh
        ...
        Serial.println("Is Charging..."); Serial.println();
...
bool fan2_speed_2_engaged = false, fan2_speed_3_engaged = false, fan2_speed_4_engaged = false;
                      ...
                      if(fan2_ON){ //whether it was in the wait or NOT, take it OFF after 5 seconds
                          if(millis() - fan_2_stop >= 4000) { // forced cooling at shutdown
                                ledcWrite(channel_2, Speed_0);  
                                 fan2_speed_2_engaged = false; fan2_speed_3_engaged = false; fan2_speed_4_engaged = false;
...
//close if power is set to zero
    ...
    */
            if(!waiting){//turn it on instantly but very silently... for the first 30 seconds
                          ...
                          if(fan1_ON){
                            if(((millis() - stat) >= 20000) && (millis() - stat) < 40000){ // for ... 20 seconds
                                if(!fan1_speed_2_engaged) {ledcWrite(channel_1, Speed_2); Serial.print("Fan 1 Speed: "); Serial.println(Speed_2); fan1_speed_2_engaged = true;  fan1_speed_4_engaged = false; } }

                            if(((millis() - stat) >= 40000) && (millis() - stat) < 280000){ // for ... 4 minutes
                                if(!fan1_speed_4_engaged){ledcWrite(channel_1, Speed_4); Serial.print("Fan 1 Speed: "); Serial.println(Speed_4); fan1_speed_4_engaged = true; fan1_speed_2_engaged = false;} } //to avoid repititions

                            if(((millis() - stat) >= 280000) && (millis() - stat) < 300000){ // for... 20 seconds
                                if(!fan1_speed_2_engaged){ledcWrite(channel_1, Speed_2); Serial.print("Fan 1 Speed: "); Serial.println(Speed_2); fan1_speed_2_engaged = true; fan1_speed_4_engaged = false; } }

                            if(((millis() - stat) >= 300000)){ //after 5 minutes, keep fan running at full blast
                                if(!fan1_speed_4_engaged){ledcWrite(channel_1, Speed_4); Serial.print("Fan 1 Speed: "); Serial.println(Speed_4); fan1_speed_4_engaged = true; fan1_speed_2_engaged = false;} } //to avoid repititions
...
if(coil_2_index != 0) {  //if toggled to a non zero integer
      ...
      if(Coil_2_ON) { //check if it hasn't alreay been turned ON
            ...
            if(!waiting2){//turn it on instantly but very silently... for the first FULL minute
                          if(!fan2_ON) { ledcWrite(channel_2, Speed_1); fan2_ON = true; /*Serial.print(F("Fan 2 Speed: ")); /*Serial.println(F(Speed_1));*/ }
                          if(fan2_ON){ //only one JAB
                            if(((millis() - stat2) >= 30000) && (millis() - stat2) < 50000){ // 20 seconds
                                if(!fan2_speed_2_engaged) {ledcWrite(channel_2, Speed_2); /*Serial.print(F("Fan 2 Speed: ")); /*Serial.println(F(Speed_2)); */ fan2_speed_2_engaged = true; fan2_speed_4_engaged = false; fan2_speed_3_engaged = false;}}
                           
                            if(((millis() - stat2) >= 50000) && (millis() - stat2) < 80000){ // 30 seconds
                                if(!fan2_speed_3_engaged) {ledcWrite(channel_2, Speed_3); /*Serial.print(F("Fan 2 Speed: ")); /*Serial.println(F(Speed_3)); */ fan2_speed_3_engaged = true; fan2_speed_4_engaged = false; fan2_speed_2_engaged = false;}} 
                            
                            if(((millis() - stat2) >= 80000) && (millis() - stat2) < 260000){ // 3 minutes
                                if(!fan2_speed_4_engaged) {ledcWrite(channel_2, Speed_4); /*Serial.print(F("Fan 2 Speed: ")); /*Serial.println(F(Speed_4)); */ fan2_speed_4_engaged = true; fan2_speed_3_engaged = false; fan2_speed_2_engaged = false;}} 
                
                            if(((millis() - stat2) >= 260000) && (millis() - stat2) < 270000){  // 10 seconds
                                if(!fan2_speed_2_engaged) {ledcWrite(channel_2, Speed_2); /*Serial.print(F("Fan 2 Speed: ")); /*Serial.println(F(Speed_2)); */ fan2_speed_2_engaged = true;  fan2_speed_4_engaged = false; fan2_speed_3_engaged = false;}} 
                          
                            if(((millis() - stat2) >= 270000) && (millis() - stat2) < 300000){ // 30 seconds
                                if(!fan2_speed_3_engaged) {ledcWrite(channel_2, Speed_3); /*Serial.print(F("Fan 2 Speed: ")); Serial.println(F(Speed_3)); */ fan2_speed_3_engaged = true; fan2_speed_4_engaged = false; fan2_speed_2_engaged = false;}} 
                            
                            if(((millis() - stat2) >= 300000)){  //after 5 minutes, keep fans in full blast
                                if(!fan2_speed_4_engaged) {ledcWrite(channel_2, Speed_4); /*Serial.print(F("Fan 2 Speed: ")); Serial.println(F(Speed_4));*/  fan2_speed_4_engaged = true; fan2_speed_3_engaged = false; fan2_speed_2_engaged = false;}} //&& to avoid repititions
   ...
   // Serial.print("Running Time: "); Serial.println(intervo); 
              ...
              if((millis() - stat) < interval) { //Serial.print("Coil "); Serial.print(CoilNum); Serial.println(" ON");
                    /*Serial.print(" Firing till ");  Serial.println((1+interval + stat)/1000);*/  /*if(reducing) waiting = true;*/
                           if(blink_counta <= 10000){ if(!is_togod) { digitalWrite(flashin_LED, HIGH); is_togod = true;}}
                      else if(blink_counta > 10000 && blink_counta <= 20000) { if(is_togod){digitalWrite(flashin_LED, LOW); is_togod = false;} }
                      else if(blink_counta > 20000) {blink_counta = 0; is_togod = false; }
