...
// --- PWM OUTPUTS
  ...
  //PWM frequency & switching tradeoffs  
//For motors / inductive loads you typically use 8–20 kHz to avoid audible noise. 
//Higher frequency reduces current ripple but increases switching losses and stresses diode/MOSFET. Start around 10–20 kHz and measure heating / EMI.

//With a 20 A load, conduction losses are often dominant (so minimize RDS(on) by driving gate to 10–12 V). 
//Keep switching edges fast enough to limit switching loss but not so fast that parasitic ringing or EMI becomes problematic.
