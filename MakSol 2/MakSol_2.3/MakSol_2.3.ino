...
// --- UNIVERSAL INDICATORs
   const int flashin_LED = 12;
   const int sound = 32;//33;

// --- POWER
 const int readBattery = 34; //39;//34;  //  26;
 const int readSolar = 36;
 const int charge_by_Solar = 13; //26; //14; //17-Dec-2022


// --- PWM OUTPUTS
  const int Coil_1 = 14; uint8_t coil_1_LED = 2; //13; // 14; //33
  const int Coil_2 = 26; uint8_t coil_2_LED = 15;
  const int fan_1 = 33;
  const int fan_2 = 25;
...
// --- TOUCH INPUTS
 ...
 const int reset = 0; //17; //const int powerLED = 34;
...
//ATTACK THE COILS first --- use direct port access to lower response to nanosex 
  ...
  Screen.clear();
    ...
    for(int count = 0; count < 2; count++) {
          Screen.showAnimation(ANIMATION, FRAMES(ANIMATION), TIME_MS(7));
      }
...
while(!activated){ 
       ...
       if(detectPress) {arrested = true; digitalWrite(flashin_LED, HIGH);}
...
*/
  ...
  Screen.clear();
  Screen.showNumber(1234567);  delay(500); // overflow     --- 2
 ...
 delay(50);
  ...
  digitalWrite(coil_1_LED, HIGH); digitalWrite(coil_2_LED, HIGH);  // TURN DEM OFF
...
uint32_t summation = 0;
  ...
  if(voltage < 10.00) { //too low battery power
     Battery_Level_Counter = 0;
     ...
     if(!isCharging) { digitalWrite(charge_by_Solar, HIGH); isCharging = true; beep(3); } // ALWAYS PREVENT DEEP DISCHARGE when there is sunshining...
...
/*
    ...
    if(heat_2_level > 0) { heat_2_level = 0; Monitor_cookingZones(heat_1_level, heat_2_level); locked_down2 = true; locked_up2=false; 
                           Screen.showNumber(heat_2_level, false); Screen.showNumber(heat_1_level, false, 1, 0); digitalWrite(coil_2_LED, LOW);}
  
  */
 }
  
  else if(voltage >= 10.00 && voltage < 11.80) {Battery_Level_Counter = 1; isPower_enough = true; end_processes = false;
        if(!LED1_ON) {digitalWrite(level1, HIGH); LED1_ON = true; }
        if(LED2_ON)  {digitalWrite(level2, LOW); LED2_ON = false; } 
        if(LED3_ON)  {digitalWrite(level3, LOW); LED3_ON = false; }
        
        if(!isCharging) { digitalWrite(charge_by_Solar, HIGH); isCharging = true; beep(3); } //PWM THE CHARGING VOLTAGE
    } 
      ...
      if(LED3_ON)  {digitalWrite(level3, LOW);  LED3_ON = false;}
       ...
       if(!isCharging) { digitalWrite(charge_by_Solar, HIGH); isCharging = true; beep(3); }
      ...
      if(voltage >= 14.35) if(isCharging) { digitalWrite(charge_by_Solar, LOW); /*isCharging = false; beep(3);*/} // PREVENT OVERCHARGE
      }
