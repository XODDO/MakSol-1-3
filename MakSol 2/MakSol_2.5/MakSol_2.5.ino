/* 15th APRIL 2023 EDITION ~650lines
  ...
  * FAN SLIGHTLY OUT OF PHASE BUT WITH DIGITAL CONTROL
  * 4-KEY NAVIGATION
 ...
 //WHENEVER POWER IS ALARMINGLY LOW < 11.4V auto stop cooking
      if(coil_1_index == 0) { 
                    ...
                    if(fan1_ON){ //whether it was in the wait or NOT, take it OFF after 5 seconds
                          if(millis() - fan_1_stop >= 3500) { // forced cooling at shutdown
      ...
      if(coil_2_index == 0) { 
                      ...
                      if(fan2_ON){ //whether it was in the wait or NOT, take it OFF after 5 seconds
                          if(millis() - fan_2_stop >= 3500) { // forced cooling at shutdown
                                digitalWrite(fan_2, LOW); waiting2 = false; fan2_ON = false;
                                fan_2_stop = 0;
          ...
          if(!waiting2) { //turn it ON if it is not in waiting state
            ...
            if(waiting2){
              ...
              //throw the fan 3-5 seconds out of start phase and make it run intermittently e.g. 1min ON, 1min OFF
                        if(!fan2_ON) if((millis() - stat2) >= 5000) { digitalWrite(fan_2, HIGH); fan2_ON = true; Serial.println("Fan 2 ON!");}
                        }
          }
          else if(Coil_2_ON) { 
        //    Serial.print("Coil "); Serial.print(CoilNum); Serial.print(" running for: "); 
